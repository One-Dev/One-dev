...
class Carrito {
    ...
    function eliminarDelCarrito($idProducto) {
        ...
        if(array_key_exists($idProducto,$aTempCarrito) and $idProducto>0) {
            $aTempCarrito[$idProducto] -=$idProducto;
        }
    ...
    function modificarDelCarrito($idProducto,$iCant) {
        $aTempCarrito = $_SESSION['carrito'];

        if(array_key_exists($idProducto,$aTempCarrito)) {
            $aTempCarrito[$idProducto] = $iCant;
        }
